    ...
    while True:
        ...
        for prior_index in range(len(outputs)):
            ...
            try:
                ...
                p_cnt=len(distances) // 10
                means = []
                for i in range(p_cnt):
                    mean = 0
                    for j in distances[i*10:(i+1)*10]:
                        mean += j
                    mean = mean/10
                    means.append(mean)
                ...
                # t_target["distance"] = means
                # print(t_target["distance"])
                # # t_target['identity'] = ["KJH", "PSC", "KMG", "LEH"]
                # t_target['identity'] = ["KJH", "PSC"]
                # t_target = t_target[t_target.distance <= threshold]
                # t_target = t_target.sort_values(by=["distance"], ascending=True).reset_index(drop=True)

                f_list = []
                db_sample["distance"] = distances
                for in_di in enumerate(means):
                    if in_di[1] <= threshold:
                        f_list.append(in_di)
                ...
                color = (0,0,255)
                if db_sample.size:
                    # name = db_sample['identity'][f_index]
                    name = ""
                    color = (0,255,0)
