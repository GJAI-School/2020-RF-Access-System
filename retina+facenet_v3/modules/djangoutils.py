...
#   DB load                                                                   #
###############################################################################
def load_db(db_path, retina_model, FLAGS, cfg_retina, facenet_model, resize_height, resize_width, RFcode):
    
    # db 폴더가 없는 경우 에러 처리
    if os.path.isdir(db_path) == True:
        db_path = db_path+RFcode+'/'
        if os.path.isdir(db_path) == True:    
            # db 얼굴 임베딩 파일 불러오기
            file_name = "representations" + RFcode +".pkl"
...
#   Detect Face                                                               #
###############################################################################
def detect_face_landm(img, retina_model, FLAGS, cfg_retina):
...
def findEuclideanL2Distance(source_representation, test_representation):
    euclidean_distance = source_representation - test_representation
